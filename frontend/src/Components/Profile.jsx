    ...
    <div className="w-full h-screen bg-zinc-900 text-white p-10">
      <div className="w-full justify-between flex items-center gap-4 mb-6">
        ...
        <div className="w-[50px] h-[50px] overflow-hidden border-2 border-white rounded-full">
          {
            user?.profilePicture === "" ? <img className="w-full h-full object-cover" src="https://t4.ftcdn.net/jpg/00/65/77/27/360_F_65772719_A1UV5kLi5nCEWI0BNLLiFaBPEkUbv5Fv.jpg" alt="" /> : <img className="w-full h-full object-cover" src={user?.profilePicture} alt="" /> 
          }
        </div>
      ...
      <h2 className="text-lg font-semibold mb-4">Posts: {user?.posts?.length}</h2>
      <h2 className="text-lg font-semibold mb-4">Followers: {user?.followers?.length}</h2>
      <h2 className="text-lg font-semibold mb-4">Following: {user?.following?.length}</h2>
      <Link to={'/edit-profile'} className="px-3 py-2 bg-blue-600 rounded-lg block mt-3 w-fit">Edit Profile</Link>
    </div>
  );
}
